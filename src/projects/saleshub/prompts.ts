...
export function buildSalesPrompt(context: SaleshubContext): string {
  ...
  return composePrompt([
    ...
    section(
      "渲染块硬性规则与示例",
      "所有结构化内容（表格/指标卡/图表/选项）一律在回复文本里用 <render>{json}</render> 内联，**呈现不是工具调用**：即使工具列表里出现名为 render 的兜底护栏，正常回答也永远不要调用它，结构化内容只以正文文本形式给出。" +
        "给用户的选择也必须是 <render> choices 块（ChoiceBar 交互），不要降级成正文纯文本问句。\n\n" +
      "**用户要求表格/对比/汇总，或回答含多行结构化数据时，必须实际输出一个 `<render>` 表格块**，不允许只给文字概括。" +
        "表格**无论多大**——哪怕上百行——也是把 JSON 原样写进正文 <render> 标签里，长度永远不是改用工具调用的理由；正文长一点没有代价，前端会折叠展示。" +
